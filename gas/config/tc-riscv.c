...
md_convert_frag_branch (fragS *fragp, segT sec)
{
  ...
  /* We have to keep the relocation for the branch over jump
     since linker optimizations, inlcuding target aligned and ex9,
			  ...
			  void *xxx ATTRIBUTE_UNUSED)
{
  ...
  /* If there is no relocation and relax is disabled, it is not necessary to
     insert R_RISCV_RELAX_ENTRY for linker do EX9 optimization.  */
  for (fixp = seginfo->fix_root; fixp; fixp = fixp->fx_next)
    if (!fixp->fx_done)
      break;
  if (!fixp && !riscv_opts.ex9)
    return;
  ...
  if (!riscv_opts.relax)
  ...
  else
    {
	 ...
	 so we set them anyway.  */
      if (riscv_opts.ex9)
	exp.X_add_number |= R_RISCV_RELAX_ENTRY_EX9_FLAG;
    }
...
riscv_aligned_cons (int idx)
{
  ...
  if (now_seg->flags & SEC_CODE
    ...
    {
      /* Use BFD_RELOC_RISCV_DATA to avoid EX9 optimization replacing data.  */
      expressionS exp;
...
/* Insert relocations to mark the region that can not do ex9 relaxation.  */

static void
riscv_no_ex9 (int mode)
{
  expressionS exp;
  ...
  if (mode == 1)
    {
      /* The begin of the region without ex9.  */
      exp.X_add_number = R_RISCV_RELAX_REGION_NO_EX9_FLAG;
      fix_new_exp (frag_now, frag_now_fix (), 0, &exp, 0,
		   BFD_RELOC_RISCV_RELAX_REGION_BEGIN);
    }
  else
    {
      /* The end of the region without ex9.  */
      exp.X_add_number = R_RISCV_RELAX_REGION_NO_EX9_FLAG;
      fix_new_exp (frag_now, frag_now_fix (), 0, &exp, 0,
