			    ...
			    asection **local_sections)
{
  ...
  for (rel = relocs; rel < relend; rel++)
	...
	continue;
      ...
      /* TODO: any better way  */
      if (r_type == R_RISCV_EXECIT_ITE)
	{
	  execit_index = rel->r_addend >> 20;
	...
	case R_RISCV_EXECIT_ITE:
	  {
		...
		bfd_vma hi20 = riscv_elf_encode_relocation (input_bfd, &reduction, relocation);
		// i = LIST_ITER(&he->vmas, &hi20, find_vma_each_cb, find_vma_final_cb);
		// BFD_ASSERT ((i - 1) < ie->entries);
		...
		else
		  {
		    ...
		    if (!is_found)
			...
			else
			  {
			    ...
			    execit_hash_t *t =
				bfd_malloc (sizeof (execit_hash_t));
			    *t = *execit_itable_array[execit_index];
			    ...
			    /* TODO: relocate the newly allocated entry only  */
			    execit.relocate_itable_done = FALSE;
			  }
		      }
		  }
			      ...
			      bfd_boolean rvc ATTRIBUTE_UNUSED)
{
  ...
  if (undefined_weak
      ...
      else
	{
	  ...
	  do_replace = andes_relax_gp_insn (&insn, rel, bias, sym);
	}
			  ...
			  bfd_boolean *again)
{
  ...
  *again = FALSE;
  ...
  /* Check relax_pass and then do the corresponding relaxation.
     ...
     Pass 7: Relax alignment round (alignment, target aligned,
	     avoid BTB miss)
     */
  ...
  bfd_boolean rvc ATTRIBUTE_UNUSED)
{
  ...
  /* For the gp relative insns, gp must be set to 4/8 bytes aligned
     address (Bug-14634).  */
  struct riscv_elf_link_hash_table *table = riscv_elf_hash_table (info);
  int gp_align;
  if ((ARCH_SIZE == 64))
    gp_align = 8;
  else
    gp_align = 4;
  if (table->gp_relative_insn
      && ((gp % gp_align) != 0))
    {
      (*_bfd_error_handler) (_("error: Please set gp to %x-byte aligned address "
			       "or turn off the gp relative instructions "
  ...
  /* TODO: find actual alignment size at runtime  */
  const int rsrv_dsa = ELF_MAXPAGESIZE; /* for DATA_SEGMENT_ALIGN  */
  bfd_signed_vma bias = symval - gp;
  bfd_signed_vma rsrv = max_alignment + reserve_size;
  if (((symval < gp) && (-bias <= (max_range - rsrv - rsrv_dsa))) ||
      ((symval >= gp) && (bias < (max_range - rsrv))))
    {
      unsigned sym = hi_reloc.hi_sym;
      do_replace = 1;
